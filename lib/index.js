...
const BASE_PACKAGES = [
  { name: '@deepseek-ai/dsh', github: 'deepseek-ai/deepseek-harness' },
  { name: '@dsh-external/dsh-vision-toolkit', github: 'Anionex/dsh-vision-toolkit' },
];
...
/** 读 GitHub latest release 的 tag（15s 超时；失败返回 null 不阻断）。 */
...
/** 收集版本表：单包失败隔离为 unknown 条目；GitHub release 超前 npm 时标记 upstream-new。 */
async function collectVersions(extra) {
  const items = [...BASE_PACKAGES];
  if (Array.isArray(extra)) {
    for (const nm of extra) {
      if (typeof nm === 'string' && nm && !items.some((i) => i.name === nm)) items.push({ name: nm, github: null });
    }
  }
  const entries = [];
  for (const item of items) {
    ...
    try {
      ...
      const githubLatest = item.github ? await fetchGithubLatest(item.github) : null;
      let status;
